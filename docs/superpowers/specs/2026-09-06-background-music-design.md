...
- If the saved choice is enabled, playback resumes on the first pointer gesture outside the music control; it never begins automatically on load.
...
- Starting and stopping use independent `fadeInMs` and `fadeOutMs` parameters, initially `200` ms each, so the author can tune either envelope independently of the playlist crossfade.
