...
| `screenshots/*` | готовые скриншоты без адресной строки, названные по slug проекта (`<slug>.webp`) |
...
- Скриншоты — из `screenshots/`; `buildImages()` в `src/build.mjs` обрабатывает их через Sharp: полный кадр в исходном разрешении и миниатюра шириной до 1000px, для каждого — меньший из AVIF/WebP (качество 90) плюс JPEG (качество 82).
